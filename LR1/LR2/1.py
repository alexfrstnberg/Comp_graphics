...
# Тут вставляємо функцію для розрахунку нових координат точок піраміди при обертанні
...
def rotate_point(point, angle, axis):
    # Конвертуємо кут обертання з градусів в радіани
    angle = angle * (pi/180)
    
    # Розпаковуємо координати точки
    x, y, z = point
    
    # Розраховуємо нові координати після обертання навколо осі X
    if axis == 'x':
        new_y = y*cos(angle) - z*sin(angle)
        new_z = y*sin(angle) + z*cos(angle)
        return (x, new_y, new_z)
    
    # Розраховуємо нові координати після обертання навколо осі Y
    elif axis == 'y':
        new_x = x*cos(angle) + z*sin(angle)
        new_z = -x*sin(angle) + z*cos(angle)
        return (new_x, y, new_z)
    
    # Розраховуємо нові координати після обертання навколо осі Z
    elif axis == 'z':
...
# Встановлення перспективної проек
...
while True:
    ...
    # Очищення екрану і буфера глибини
    glClear(GL_COLOR_BUFFER_BIT|GL_DEPTH_BUFFER_BIT)

    # Рисування граней піраміди
    glBegin(GL_LINES)
...
# Збереження зображення (якщо потрібно)
